...
%%
%% @end
-spec start_link(Params) -> {ok, pid()} | {error, any()} when Params :: ieee_parameters().
start_link(Params) ->
    gen_server:start_link({local, ?MODULE}, ?MODULE, Params, []).

%% @doc Same as start_link/1 but no link is created
%% @end
-spec start(Params) -> {ok, pid()} | {error, any()} when Params :: ieee_parameters().
start(Params) ->
...
transmission(Frame, Ranging) ->
    ...
    case FH of
        ...
        _ ->
            gen_server:call(?MODULE, {tx, Frame, Ranging}, infinity)
    end.

%% @doc Performs a reception on the IEEE 802.15.4 stack
%% @deprecated This function will be deprecated
%% @end
-spec reception() -> Result when Result :: {ok, frame()} | {error, atom()}.
reception() ->
...
%% @equiv rx_on(0)
%% @end
-spec rx_on() -> Result when Result :: ok | {error, atom()}.
rx_on() ->
...
%% '''
%% @end
-spec rx_on(Ranging) -> ok when Ranging :: ?DISABLED | ?ENABLED.
rx_on(Ranging) ->
...
init(Params) ->
    ...
    DutyCycleState = gen_duty_cycle:start(Params#ieee_parameters.duty_cycle, PhyMod),
...
handle_call({rx_on, RangingFlag}, _From, State) ->
    #{duty_cycle := DCState, input_callback := Callback} = State,
    NewCallback =
        fun(Frame, LQI, _PRF, Sec, _PreambleRep, _DataRate, Rng) ->
    ...
    case gen_duty_cycle:turn_on(DCState, NewCallback, RangingFlag) of
        ...
        {error, NewDutyCycleState, Error} ->
            {reply, {error, Error}, State#{duty_cycle => NewDutyCycleState, ranging := RangingFlag}}
    end;
...
handle_call({reset, SetDefaultPIB}, _From, State) ->
    #{
        ...
        State,
    NewState =
        case SetDefaultPIB of
            true ->
                PhyMod:write(panadr, #{pan_id => <<16#FFFF:16>>, short_addr => <<16#FFFF:16>>}),
