...
encode_ack(FramePending, Seqnum) ->
    FC = build_frame_control(#frame_control{
        frame_type = ?FTYPE_ACK,
...
build_mac_header(FrameControl, MacHeader) ->
    ...
    DestAddrFields =
        case FrameControl#frame_control.dest_addr_mode of
            ?NONE ->
                <<>>;
            _ ->
                <<DestPan/bitstring, DestAddr/bitstring>>
        end,
    ...
    SrcAddrFields =
        case
            {
                FrameControl#frame_control.src_addr_mode,
        ...
        of
            ...
            {_, ?DISABLED, _} ->
                <<SrcPan/bitstring,
                    % if no compression is applied on PANID and SRC addr is present
                    SrcAddr/bitstring>>;
            {_, ?ENABLED, ?NONE} ->
                <<SrcPan/bitstring,
                    % if there is a compression of the PANID but the dest addr isn't present
                    SrcAddr/bitstring>>;
            {_, ?ENABLED, _} ->
                % if there is a compression of the PANID and the dest addr is present
                <<SrcAddr/bitstring>>
        end,
...
decode_rest(
    #frame_control{frame_type = ?FTYPE_ACK} = FrameControl,
    Seqnum,
    % Might cause an issue if piggybacking is used (allowed in IEEE 802.15.4?)
    _Rest
) ->
...
-spec decode_mac_header(DestAddrMode, SrcAddrMode, PanIdCompr, Bits) ->
    {DestPAN, DestAddr, SrcPAN, SrcAddr, Payload}
when
    DestAddrMode :: flag(),
    SrcAddrMode :: flag(),
...
decode_mac_header(
    ...
    ?EXTENDED,
    ?DISABLED,
    <<DestPAN:16/bitstring, DestAddr:64/bitstring, SrcPAN:16/bitstring, SrcAddr:64/bitstring, Payload/bitstring>>
) ->
    {DestPAN, DestAddr, SrcPAN, SrcAddr, Payload};
decode_mac_header(
    ?EXTENDED,
    ?EXTENDED,
    ?ENABLED,
    <<DestPAN:16/bitstring, DestAddr:64/bitstring, SrcAddr:64/bitstring, Payload/bitstring>>
) ->
...
decode_mac_header(
    ...
    ?SHORT_ADDR,
    ?DISABLED,
    <<DestPAN:16/bitstring, DestAddr:64/bitstring, SrcPAN:16/bitstring, SrcAddr:16/bitstring, Payload/bitstring>>
) ->
    {DestPAN, DestAddr, SrcPAN, SrcAddr, Payload};
decode_mac_header(
    ?EXTENDED,
    ?SHORT_ADDR,
    ?ENABLED,
    <<DestPAN:16/bitstring, DestAddr:64/bitstring, SrcAddr:16/bitstring, Payload/bitstring>>
) ->
    {DestPAN, DestAddr, DestPAN, SrcAddr, Payload};
decode_mac_header(
    ?EXTENDED,
    ?NONE,
    _,
    <<DestPAN:16/bitstring, DestAddr:64/bitstring, Payload/bitstring>>
) ->
...
decode_mac_header(
    ...
    ?EXTENDED,
    ?DISABLED,
    <<DestPAN:16/bitstring, DestAddr:16/bitstring, SrcPAN:16/bitstring, SrcAddr:64/bitstring, Payload/bitstring>>
) ->
    {DestPAN, DestAddr, SrcPAN, SrcAddr, Payload};
decode_mac_header(
    ?SHORT_ADDR,
    ?EXTENDED,
    ?ENABLED,
    <<DestPAN:16/bitstring, DestAddr:16/bitstring, SrcAddr:64/bitstring, Payload/bitstring>>
) ->
    {DestPAN, DestAddr, DestPAN, SrcAddr, Payload};
decode_mac_header(
    ?SHORT_ADDR,
    ?SHORT_ADDR,
    ?DISABLED,
    <<DestPAN:16/bitstring, DestAddr:16/bitstring, SrcPAN:16/bitstring, SrcAddr:16/bitstring, Payload/bitstring>>
) ->
    {DestPAN, DestAddr, SrcPAN, SrcAddr, Payload};
decode_mac_header(
    ?SHORT_ADDR,
    ?SHORT_ADDR,
    ?ENABLED,
    <<DestPAN:16/bitstring, DestAddr:16/bitstring, SrcAddr:16/bitstring, Payload/bitstring>>
) ->
    {DestPAN, DestAddr, DestPAN, SrcAddr, Payload};
decode_mac_header(
    ?SHORT_ADDR,
    ?NONE,
    _,
    <<DestPAN:16/bitstring, DestAddr:16/bitstring, Payload/bitstring>>
) ->
    {DestPAN, DestAddr, <<>>, <<>>, Payload};
decode_mac_header(
    ?NONE,
    ?EXTENDED,
    _,
    <<SrcPAN:16/bitstring, SrcAddr:64/bitstring, Payload/bitstring>>
) ->
...
decode_mac_header(
    ...
    ?SHORT_ADDR,
    _,
    <<SrcPAN:16/bitstring, SrcAddr:16/bitstring, Payload/bitstring>>
) ->
    {<<>>, <<>>, SrcPAN, SrcAddr, Payload};
...
% @end
%-------------------------------------------------------------------------------
-spec build_frame_control(FrameControl) -> <<_:16>> when FrameControl :: frame_control().
...
% @end
%-------------------------------------------------------------------------------
-spec decode_frame_control(FC) -> frame_control() when FC :: <<_:16>>.
decode_frame_control(FC) ->
    <<_:1, PanIdCompr:1, AckReq:1, FramePending:1, SecEn:1, FrameType:3, SrcAddrMode:2, FrameVersion:2, DestAddrMode:2, _:2>> =
        FC,
    #frame_control{
        frame_type = FrameType,
        sec_en = SecEn,
