...
-export([pkt_encapsulation/2, create_iphc_pckt/2, fragment_ipv6_packet/2,
         reassemble_datagram/2, reassemble_datagrams/1, reassemble/2, build_iphc_header/1,
         get_ipv6_pkt/2, datagram_info/1, compress_ipv6_header/1, build_datagram_pckt/2,
         build_firstFrag_pckt/5, convert_iphc_tuple_to_bin/1, get_ipv6_pckt_info/1,
         get_ipv6_payload/1, trigger_fragmentation/2, map_to_binary/1, tuple_list_to_binary/1,
         binary_to_lis/1, decompress_ipv6_header/2, get_default_LL_add/1, encode_integer/1,
         tuple_to_bin/1, build_frag_header/1, get_next_hop/1, print_as_binary/1, hex_to_binary/1,
         complete_with_padding/1, generate_chunks/0, build_mesh_header/1, get_mesh_info/1,
         contains_mesh_header/1, build_first_frag_header/1, get_unc_ipv6/1]).
...
encode_list_to_bin(List) ->
    ...
    list_to_bin(EncodedValues).%% Use to optimize encoding
...
compress_ipv6_header(Ipv6Pckt) ->
    ...
    {CID, UpdateMap0, UpdatedList0} =
        process_cid(SourceAddress,
                    DestAddress,
                    Map,
                    List), % first one because context identifier extension should follow DAM
...
process_tf(TrafficClass, FlowLabel, CarrInlineMap, CarrInlineList) ->
    <<DSCP:6, ECN:2>> = <<TrafficClass:8>>, % TrafficClass integer to a binary
...
process_nh(NextHeader, CarrInlineMap, CarrInlineList) when NextHeader == ?UDP_PN ->
    ...
    %L = [Bin],
    %UpdatedList = [CarrInlineList, L],
    {1, CarrInlineMap, CarrInlineList}; % UDP %TODO check compression for UDP
...
process_sam(SAC, _, SrcAdd, CarrInlineMap, CarrInlineList) when SAC == 0 ->
    ...
    case SrcAddBits of
        ...
        <<?LINK_LOCAL_PREFIX:16, 0:48, 16#000000FFFE00:48, _:16>> ->
            ...
            {2#10,
             UpdatedMap,
             UpdatedList}; % the first 112 bits are elided, last 16 IID bits are carried in-line
        <<?LINK_LOCAL_PREFIX:16, 0:48, _:64>> ->
            ...
            {2#01,
             UpdatedMap,
             UpdatedList}; % the first 64 bits are elided, last 64 bits (IID) are carried in-line
        _ ->
            Bin = <<SrcAdd:128>>,
            L = [Bin],
            UpdatedList = [CarrInlineList, L],
...
process_sam(SAC, 1, SrcAdd, CarrInlineMap, CarrInlineList) when SAC == 1 ->
    ...
    case SrcAddBits of
        ...
        <<0:128>> ->
            {2#11,
             CarrInlineMap,
             CarrInlineList}; % the address is fully elided and derived from the context
        <<_:16, _:48, 16#000000FFFE00:48, _:16>> ->
            ...
            {2#10,
             UpdatedMap,
             UpdatedList}; % the first 64 bits are derived from the context, last 16 IID bits are carried in-line
        <<_:16, _:48, _:64>> -> %TODO how to represente first context 64 bit
            ...
            {2#01,
             UpdatedMap,
             UpdatedList} % the first 64 bits are derived from the context, last 64 bits IID are carried in-line
    end.
...
process_dam(0, 0, _, DstAdd, CarrInlineMap, CarrInlineList) ->
    ...
    case DestAddBits of
        ...
        <<?LINK_LOCAL_PREFIX:16, 0:48, _:24, 16#FFFE:16, _:24>> ->
            {2#11,
             CarrInlineMap,
             CarrInlineList}; % MAC address is split into two 24-bit parts, FFFE is inserted in the middle
        <<?LINK_LOCAL_PREFIX:16, 0:48, 16#000000FFFE00:48, _:16>> ->
            Bin = <<Last16Bits:16>>,
            L = [Bin],
            UpdatedList = [CarrInlineList, L],
            ...
            {2#10,
             UpdatedMap,
             UpdatedList}; % the first 112 bits are elided, last 16 bits are in-line
        <<?LINK_LOCAL_PREFIX:16, _:112>> ->
            ...
            {2#01,
             UpdatedMap,
             UpdatedList}; % the first 64 bits are elided, last 64 bits are in-line
        _ ->
            Bin = <<DstAdd:128>>,
            L = [Bin],
            UpdatedList = [CarrInlineList, L],
...
process_dam(0, 1, 1, DstAdd, CarrInlineMap, CarrInlineList) ->
    ...
    case DestAddBits of
        ...
        <<?GLOBAL_PREFIX_1:16,
          _:48,
          16#000000FFFE00:48,
          _:16>> ->  % MAC address is split into two 24-bit parts, FFFE is inserted in the middle
            Bin = <<Last16Bits:16>>,
            L = [Bin],
            UpdatedList = [CarrInlineList, L],
            UpdatedMap = CarrInlineMap#{"DAM" => Last16Bits},
...
process_dam(1, 0, _, DstAdd, CarrInlineMap, CarrInlineList) ->
    ...
    case DestAddBits of
        <<?MULTICAST_PREFIX:16, 0:104, _:8>> -> % ff02::00XX.
            Bin = <<Last8Bits:8>>,
            L = [Bin],
            UpdatedList = [CarrInlineList, L],
...
process_udp_checksum(Checksum, CarriedInline) ->
    case Checksum % TODO check checksum values
    of
        0 ->
            {1, CarriedInline};
        _ -> %Checksum is carried inline
            L = [<<Checksum:16>>],
...
build_frag_header(FragHeader) ->
    #frag_header{frag_type = FragType,
...
frag_process(CompIpv6Pckt, DatagramTag, PacketLen, Offset, Acc) ->
    ...
    case Offset of
        0 ->
            Header =
                build_first_frag_header(#frag_header{frag_type = ?FRAG1_DHTYPE,
                                                     datagram_size = PacketLen,
                                                     datagram_tag = DatagramTag,
                                                     datagram_offset = Offset});
        _ ->
            Header =
                build_frag_header(#frag_header{frag_type = ?FRAGN_DHTYPE,
                                               datagram_size = PacketLen,
                                               datagram_tag = DatagramTag,
                                               datagram_offset = Offset})
    end,
...
decompress_ipv6_header(CompressedPacket, EUI64) ->
    ...
    DecompressedPckt.    %{TrafficClass, FlowLabel, NextHeader, HopLimit, SourceAddress, DestAddress, Payload}.
...
datagram_info(Fragment) ->
    ...
    case FragType of
        ?FRAG1_DHTYPE ->
            ...
            FragInfo =
                #datagramInfo{fragtype = FragType,
...
reassemble_datagrams(Fragments) when is_list(Fragments) ->
    ...
    DatagramMap =
        maps:put(Tag, Datagram, ?DATAGRAMS_MAP), % add retrieve info to the datagram map
...
process_fragments([], Map, ReassembledPacket) ->
    {ReassembledPacket,
     Map};  % when the list is empty, returns the last payload and the final map state
...
process_fragment(<<?FRAGN_DHTYPE:5, Size:11, Tag:16, Offset:8, Payload/binary>>, Map) ->
    case maps:find(Tag, Map) of
        {ok, OldDatagram} ->
            ...
            UpdatedDatagram =
                OldDatagram#datagram{cmpt = UpdatedCmpt,
                                     fragments = NewFragments}, % update datagram
...
reassemble(Tag, UpdatedMap) ->
    ...
    ReassembledPacket =
        lists:foldl(fun({_Offset, Payload}, Acc) ->
                       <<Acc/binary, Payload/binary>> % append new payload to the end
                    end,
                    <<>>,
                    SortedFragments), %% <<>> is the initial value of the accumulator
...
build_mesh_header(MeshHeader) ->
    ...
    <<?MESH_DHTYPE:2,
      ...
      FBit:1,
      HopsLeft:4,
      OriginatorAddress/binary,
      FinalDestinationAddress/binary>>.
    ...
    case {V, F} of
        {0, 0} ->
            <<?MESH_DHTYPE:2,
              ...
              FBit:1,
              HopsLeft:4,
              OriginatorAddress:64,
              FinalDestinationAddress:64,
              Data/bitstring>> =
                Datagram;
        {0, 1} ->
            <<?MESH_DHTYPE:2,
...
generate_chunk(N, Size) ->
    ...
    Padding = list_to_binary(lists:duplicate(PaddingSize, $a)), % Filler character 'a'
