...
-export([tx/0, tx_unc_ipv6/0, tx_iphc_pckt/0, tx_frag_iphc_pckt/0, tx_big_payload/0,
         tx_with_udp/0, tx_msh_iphc_pckt/0, tx_msh_frag_iphc_pckt/0, msh_pckt_tx/0, rx/0]).
-export([rx_off/0]).
% Benchmarking
...
tx_msh_frag_iphc_pckt() ->
    ...
    FragHeader =
        #frag_header{frag_type = ?FRAG1_DHTYPE,
...
tx_with_udp() ->
    IPv6Header =
        #ipv6_header{version = 6,
                     traffic_class = 0,
                     flow_label = 0,
                     payload_length = ?PayloadLength + 4, % 4 bytes for the UDP header
                     next_header = 17,
                     hop_limit = 64,
                     source_address = ?Node1Address,
                     destination_address = ?Node2Address},
    UdpHeader =
        #udp_header{source_port = 1025,
...
-spec rx_callback(Frame, LinkQuality, Security, Ranging) -> ok
    when Frame :: frame(),
         LinkQuality :: integer(),
...
tx(N, Total, Success, Error) ->
    ...
    case ieee802154:transmission({#frame_control{pan_id_compr = ?ENABLED, ack_req = ?ENABLED},
...
tx_benchmark() ->
    ...
    io:format("------------------- Report -------------------~n"),
    io:format("Sent ~w frames - Success rate ~.3f (~w/~w) - Error rate ~.3f (~w/~w)~n",
              [Total, Success / Total, Success, Total, Error / Total, Error, Total]),
    io:format("Data rate ~.1f b/s - ~w b in ~w s ~n",
...
start(_Type, _Args) ->
    ...
    ieee802154:start(#ieee_parameters{duty_cycle = duty_cycle_non_beacon,
    ...
    NodeMacAddress = <<16#CAFEDECA00000002:64>>,
    lowpan_layer:start(#{node_mac_addr => NodeMacAddress}),
    %tx(),
    {ok, Supervisor}.
