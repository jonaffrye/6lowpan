...
% CSMA constants
-define(MACMAXFRAMERETRIES, 10).
% works with 2000 µs but calculations give me 4081µs
-define(MACACKWAITDURATION, 4000).
% -define(MACACKWAITDURATION, 2000).  % works with 2000 µs but calculations give me 4081µs

%--- Types ---------------------------------------------------------------------

%--- Record types
-record(ieee_parameters, {
    duty_cycle = duty_cycle_non_beacon :: module(),
    phy_layer = pmod_uwb :: module(),
    input_callback = fun(_, _, _, _) -> ok end :: input_callback()
}).
-record(ranging_informations, {
    ranging_received = ?NO_RANGING_REQUESTED :: ranging_received() | boolean(),
...
-type ranging_received() ::
    ?NO_RANGING_REQUESTED | ?RANGING_REQUESTED_BUT_NOT_SUPPORTED | ?RANGING_ACTIVE.
% PHY_HEADER_ONLY no used in our case
-type ranging_tx() :: ?NON_RANGING | ?ALL_RANGING.
% *** indicates unusefull parameters for higher layers for now
-type input_callback() ::
    fun(
        (
            Frame :: mac_frame:frame(),
            LQI :: integer(),
            Security :: security(),
