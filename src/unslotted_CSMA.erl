...
-define(AUNITBACKOFFPERIOD,
    % The number of symbols forming the basic time period used in CSMA-CA (src. IEEE 802.15.4 stdMA-CA (src. IEEE 802.15.4 std.)
    20
).
...
-spec tx(State, Frame, Pib, TxOpts) ->
    {ok, State} | {error, State, channel_access_failure}
when
    State :: map(),
    Frame :: bitstring(),
    Pib :: pib_state(),
...
tx(#{phy_layer := PhyMod} = State, Frame, Pib, TxOpts) ->
    ...
    Ret =
        case try_cca(PhyMod, 0, MacMinBE, MacMaxBE, MacMaxCSMABackoffs) of
            ok ->
...
cca(PhyMod) ->
    case PhyMod:reception() of
        ...
        rxprd ->
            error;
        rxsfdd ->
            % theoritically, this should cover any frame rx (i.e. channel is busy)
            error;
        _ ->
            % In case you receive a frame -> ? Could this happen ?
            error
    end.
