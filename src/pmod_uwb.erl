...
-compile(
    {nowarn_unused_function, [
        debug_read/2, debug_write/2, debug_write/3, debug_bitstring/1, debug_bitstring_hex/1
    ]}
).
...
-define(READ_ONLY_REG_FILE(RegFileID),
    RegFileID == dev_id;
    RegFileID == sys_time;
    RegFileID == rx_finfo;
    RegFileID == rx_buffer;
...
-spec reception() -> Result when Result :: {integer(), bitstring()} | {error, any()}.
reception() ->
    reception(false).
...
wait_for_reception() ->
    ...
    case read(sys_status) of
        ...
        _ ->
            error({error_wait_for_reception})
    end.
...
set_frame_timeout(Timeout) ->
    ...
    % enable receive wait timeout
    write(sys_cfg, #{rxwtoe => 2#1}).
...
-spec set_preamble_timeout(Timeout) -> ok when Timeout :: pos_integer().
...
first_path_power_level() ->
    ...
    A = 113.77,
    N = preamble_acc(),
    10 * math:log10((math:pow(F1, 2) + math:pow(F2, 2) + math:pow(F3, 2)) / math:pow(N, 2)) -
        A.
...
config(Bus) ->
    ...
    % Setting preamble symbols to 1024
    write_reg(Bus, tx_fctrl, #{txpsr => 2#10}).
...
tx(
    ...
    % DW1000 automatically adds the 2 bytes CRC
    DataLength = byte_size(Data) + 2,
    write_tx_data(Bus, Data),
    % Setting the options of the transmission
    case Wait4resp of
        ?ENABLED ->
            write_reg(Bus, ack_resp_t, #{w4r_tim => W4rTim});
        _ ->
            ok
    end,
    case TxDlys of
        ?ENABLED ->
            write_reg(Bus, dx_time, #{dx_time => TxDelay});
        _ ->
            ok
    end,
    write_reg(
    ...
    write_reg(
        ...
        #{
            txstrt => 2#1,
            wait4resp => Wait4resp,
            % start transmission and some options
            txdlys => TxDlys
        }
    ).
...
get_rx_data(Bus) ->
    ...
    Frame = read_rx_data(Bus, FrameLength - 2),
    {FrameLength, Frame}.
...
header(Op, RegFileID, SubRegister, 2) ->
    <<(rw(Op)):1, 2#1:1, (regFile(RegFileID)):6, 2#0:1, (subReg(SubRegister)):7>>;
header(Op, RegFileID, SubRegister, 3) ->
    <<_:1, HighOrder:8, LowOrder:7>> = <<(subReg(SubRegister)):16>>,
    <<(rw(Op)):1, 2#1:1, (regFile(RegFileID)):6, 2#1:1, LowOrder:7, HighOrder:8>>.

%% @private
%% @doc Read the values stored in a register file
read_reg(Bus, lde_ctrl) ->
    read_reg(Bus, lde_if);
read_reg(Bus, lde_if) ->
    lists:foldl(
        fun(Elem, Acc) ->
...
read_sub_reg(Bus, RegFileID, SubRegister) ->
    ...
    [Resp] =
        grisp_spi:transfer(Bus, [{?SPI_MODE, Header, HeaderSize, subRegSize(SubRegister)}]),
        ...
        fun(SubRegister, Val) ->
            CurrVal =
                maps:get(
                    ...
                    read_reg(
                        Bus,
                        % ? can the read be done before ? Maybe but not assured that no values changes after a write in the register
                        RegFileID
                    )
                ),
            Body =
                case CurrVal of
                    V when is_map(V) ->
                        reg(
                            encode,
                            SubRegister,
                            maps:merge_with(fun(_Key, _Old, New) -> New end, CurrVal, Val)
                        );
                    _ ->
                        reg(encode, SubRegister, #{SubRegister => Val})
                end,
            Header = header(write, RegFileID, SubRegister),
            % debug_write(RegFileID, SubRegister, Body),
            _ = grisp_spi:transfer(
                Bus,
                [{?SPI_MODE, <<Header/binary, Body/binary>>, 2 + subRegSize(SubRegister), 0}]
            )
        end,
        Value
    ),
...
write_reg(Bus, RegFileID, Value) ->
    ...
    _ = grisp_spi:transfer(
        Bus,
        [{?SPI_MODE, <<Header/binary, Body/binary>>, 1 + regSize(RegFileID), 0}]
    ),
    ok.
...
write_tx_data(Bus, Value) when is_binary(Value), bit_size(Value) < 1025 ->
    ...
    _ = grisp_spi:transfer(
        Bus,
        [{?SPI_MODE, <<Header/binary, Value/binary>>, 1 + Length, 0}]
    ),
    ok.
...
reg(encode, SubRegister, Value) when ?READ_ONLY_SUB_REG(SubRegister) ->
    error({writing_read_only_sub_register, SubRegister, Value});
reg(decode, dev_id, Resp) ->
    <<RIDTAG:16, Model:8, Ver:4, Rev:4>> = reverse(Resp),
    #{
        ridtag => integer_to_list(RIDTAG, 16),
        model => Model,
        ver => Ver,
        rev => Rev
    };
...
reg(encode, panadr, Val) ->
    #{pan_id := PanId, short_addr := ShortAddr} = Val,
    reverse(<<PanId:16/bitstring, ShortAddr:16/bitstring>>);
reg(decode, sys_cfg, Resp) ->
    <<FFA4:1, FFAR:1, FFAM:1, FFAA:1, FFAD:1, FFAB:1, FFBC:1,
        % bits 7-0
        FFEN:1, FCS_INIT2F:1, DIS_RSDE:1, DIS_PHE:1, DIS_DRXB:1, DIS_FCE:1, SPI_EDGE:1, HIRQ_POL:1,
        % bits 15-8
        FFA5:1, _:1, RXM110K:1, _:3, DIS_STXP:1,
        % bits 23-16
        PHR_MODE:2, AACKPEND:1, AUTOACK:1, RXAUTR:1, RXWTOE:1,
        % bits 31-24
        _:4>> =
        Resp,
    #{
...
reg(encode, sys_cfg, Val) ->
    ...
    <<FFA4:1, FFAR:1, FFAM:1, FFAA:1, FFAD:1, FFAB:1, FFBC:1,
        ...
        FFEN:1, FCS_INIT2F:1, DIS_RSDE:1, DIS_PHE:1, DIS_DRXB:1, DIS_FCE:1, SPI_EDGE:1, HIRQ_POL:1,
        % bits 15-8
        FFA5:1, 2#0:1, RXM110K:1, 2#0:3, DIS_STXP:1,
        % bits 23-16
        PHR_MODE:2, AACKPEND:1, AUTOACK:1, RXAUTR:1, RXWTOE:1,
        % bits 31-24
        2#0:4>>;
...
reg(encode, tx_fctrl, Val) ->
    #{
        ...
        Val,
    reverse(<<IFSDELAY:8, TXBOFFS:10, PE:2, TXPSR:2, TXPRF:2, TR:1, TXBR:2, R:3, TFLE:3, TFLEN:7>>);
...
reg(encode, sys_ctrl, Val) ->
    #{
        ...
        Val,
    <<WAIT4RESP:1, TRXOFF:1, 2#0:2, CANSFCS:1, TXDLYS:1, TXSTRT:1,
        % bits 7-0
        SFCST:1, 2#0:6, RXDLYE:1,
        % bits 15-8
        RXENAB:1,
        % bits 23-16
        2#0:8, 2#0:7,
...
reg(decode, sys_mask, Resp) ->
    <<MTXFRS:1, MTXPHS:1, MTXPRS:1, MTXFRB:1, MAAT:1, MESYNCR:1, MCPLOCK:1,
        % bits 7-0
        Reserved0:1, MRXFCE:1, MRXFCG:1, MRXDFR:1, MRXPHE:1, MRXPHD:1, MLDEDON:1, MRXSFDD:1,
        % bits 15-8
        MRXPRD:1, MSLP2INIT:1, MGPIOIRQ:1, MRXPTO:1, MRXOVRR:1, Reserved1:1, MLDEERR:1, MRXRFTO:1,
        % bits 23-16
        MRXRFSL:1, Reserved2:2, MAFFREJ:1, MTXBERR:1, MHPDDWAR:1, MPLLHILO:1, MCPLLLL:1,
        % bits 31-24
        MRFPLLLL:1>> =
        Resp,
    #{
...
reg(decode, sys_status, Resp) ->
    <<TXFRS:1, TXPHS:1, TXPRS:1, TXFRB:1, AAT:1, ESYNCR:1, CPLOCK:1,
        % bits 7-0
        IRQS:1, RXFCE:1, RXFCG:1, RXDFR:1, RXPHE:1, RXPHD:1, LDEDONE:1, RXSFDD:1,
        % bits 15-8
        RXPRD:1, SPL2INIT:1, GPIOIRQ:1, RXPTO:1, RXOVRR:1, Reserved0:1, LDEERR:1, RXRFTO:1,
        % bits 23-16
        RXRFSL:1, ICRBP:1, HSRBP:1, AFFREJ:1, TXBERR:1, HPDWARN:1, RXSFDTO:1, CLCKPLL_LL:1,
        % bits 31-24
        RFPLL_LL:1, Reserved1:5, TXPUTE:1, RXPREJ:1,
        % bits 39-32
        RXRSCS:1>> =
        Resp,
    #{
...
reg(decode, rx_fqual, Resp) ->
    <<CIR_PWR:16, PP_AMPL3:16, FP_AMPL2:16, STD_NOISE:16>> = Resp,
    #{
        cir_pwr => CIR_PWR,
        pp_ampl3 => PP_AMPL3,
        fp_ampl2 => FP_AMPL2,
...
reg(decode, rx_ttcki, Resp) ->
    <<RXTTCKI:32>> = reverse(Resp),
    #{rxttcki => RXTTCKI};
reg(decode, rx_ttcko, Resp) ->
    <<_:1, RCPHASE:7, RSMPDEL:8, _:5, RXTOFS:19>> = reverse(Resp),
    #{
        rcphase => RCPHASE,
        rsmpdel => RSMPDEL,
        rxtofs => RXTOFS
...
reg(decode, rx_time, Resp) ->
    <<RX_RAWST:40, FP_AMPL1:16, FP_INDEX:16, RX_STAMP:40>> = reverse(Resp),
    #{
        rx_rawst => RX_RAWST,
        fp_ampl1 => FP_AMPL1,
        fp_index => FP_INDEX,
...
reg(decode, tx_time, Resp) ->
    <<TX_RAWST:40, TX_STAMP:40>> = reverse(Resp),
...
reg(decode, sys_state, Resp) ->
    <<_:8, _:4, PMSC_STATE:4, _:3, RX_STATE:5, _:4, TX_STATE:4>> = reverse(Resp),
    #{
...
reg(decode, ack_resp_t, Resp) ->
    <<ACK_TIME:8, _:4, W4R_TIME:20>> = reverse(Resp),
    #{ack_tim => ACK_TIME, w4r_tim => W4R_TIME};
reg(encode, ack_resp_t, Val) ->
    #{ack_tim := ACK_TIME, w4r_tim := W4R_TIME} = Val,
    reverse(<<ACK_TIME:8, 2#0:4, W4R_TIME:20>>);
reg(decode, rx_sniff, Resp) ->
    <<Reserved0:16, SNIFF_OFFT:8, Reserved1:4, SNIFF_ONT:4>> = reverse(Resp),
    #{
        res0 => Reserved0,
...
reg(encode, rx_sniff, Val) ->
    #{
        ...
        Val,
    reverse(<<Reserved0:16, SNIFF_OFFT:8, Reserved1:4, SNIFF_ONT:4>>);
% Smart transmit power control (cf. user manual p 104)
reg(decode, tx_power, Resp) ->
    <<BOOSTP125:8, BOOSTP250:8, BOOSTP500:8, BOOSTNORM:8>> = reverse(Resp),
    #{
        boostp125 => BOOSTP125,
        boostp250 => BOOSTP250,
        boostp500 => BOOSTP500,
...
reg(encode, tx_power, Val) ->
    ...
    case Val of
        #{tx_power := ValToEncode} ->
            reverse(<<ValToEncode:32>>);
        #{
            boostp125 := BOOSTP125,
            boostp250 := BOOSTP250,
            boostp500 := BOOSTP500,
            ...
            reverse(<<BOOSTP125:8, BOOSTP250:8, BOOSTP500:8, BOOSTNORM:8>>)
    end;
...
reg(encode, usr_sfd, Value) ->
    #{usr_sfd := USR_SFD} = Value,
    reverse(<<USR_SFD:(8 * 41)>>);
reg(decode, usr_sfd, Resp) ->
    <<USR_SFD:(8 * 41)>> = reverse(Resp),
    #{usr_sfd => USR_SFD};
% AGC_CTRL is a complex register with reserved bits that can't be written
reg(encode, agc_ctrl1, Val) ->
    #{res := Reserved, dis_am := DIS_AM} = Val,
    reverse(<<Reserved:15, DIS_AM:1>>);
reg(encode, agc_tune1, Val) ->
    #{agc_tune1 := AGC_TUNE1} = Val,
    reverse(<<AGC_TUNE1:16>>);
reg(encode, agc_tune2, Val) ->
    #{agc_tune2 := AGC_TUNE2} = Val,
    reverse(<<AGC_TUNE2:32>>);
reg(encode, agc_tune3, Val) ->
    #{agc_tune3 := AGC_TUNE3} = Val,
    reverse(<<AGC_TUNE3:16>>);
reg(decode, agc_ctrl, Resp) ->
    <<_:4, EDV2:9, EDG1:5,
        % AGC_STAT1 (RP => don't save reserved bits)
        _:6,
        % Reserved 4
        _:80,
        % AGC_TUNE3
        ...
        _:48,
        % AGC_TUNE1
        AGC_TUNE1:16, Reserved0:15,
        % AGC_CTRL1 (RW => save reserved bits)
        DIS_AM:1,
        % Reserved 1
        _:16>> =
        reverse(Resp),
...
reg(encode, ec_ctrl, Val) ->
    #{
        ...
        Val,
    reverse(
        <<Reserved:20, OSTRM:1, WAIT:8, PLLLDT:1, OSRSM:1,
            % EC_CTRL
            OSTSM:1>>
    );
reg(decode, ext_sync, Resp) ->
    <<_:26,
        ...
        OFFSET_EXT:6,
        % EC_RXTC
        RX_TS_EST:32, Reserved:20, OSTRM:1, WAIT:8, PLLLDT:1, OSRSM:1,
        % EC_CTRL
        OSTSM:1>> =
        reverse(Resp),
    #{
...
reg(encode, gpio_mode, Val) ->
    #{
        ...
        Val,
    reverse(
        <<2#0:8, MSGP8:2, MSGP7:2, MSGP6:2, MSGP5:2, MSGP4:2, MSGP3:2, MSGP2:2, MSGP1:2, MSGP0:2,
            % GPIO_MODE
            2#0:6>>
    );
reg(encode, gpio_dir, Val) ->
    #{
        ...
        Val,
    reverse(
        <<2#0:11, GDM8:1, 2#0:3, GDP8:1, GDM7:1, GDM6:1, GDM5:1, GDM4:1, GDP7:1, GDP6:1, GDP5:1, GDP4:1, GDM3:1, GDM2:1, GDM1:1, GDM0:1, GDP3:1, GDP2:1, GDP1:1,
            % GPIO2_DIR
            GDP0:1>>
    );
reg(encode, gpio_dout, Val) ->
    #{
        ...
        Val,
    reverse(
        <<2#0:11, GOM8:1, 2#0:3, GOP8:1, GOM7:1, GOM6:1, GOM5:1, GOM4:1, GOP7:1, GOP6:1, GOP5:1, GOP4:1, GOM3:1, GOM2:1, GOM1:1, GOM0:1, GOP3:1, GOP2:1, GOP1:1,
            % GPIO_DOUT
            GOP0:1>>
    );
reg(encode, gpio_irqe, Val) ->
    #{
        ...
        Val,
    reverse(
        <<2#0:23, GIRQE8:1, GIRQE7:1, GIRQE6:1, GIRQE5:1, GIRQE4:1, GIRQE3:1, GIRQE2:1, GIRQE1:1,
            % GPIO_IRQE
            GIRQE0:1>>
    );
reg(encode, gpio_isen, Val) ->
    #{
        ...
        Val,
    reverse(
        <<2#0:23, GISEN8:1, GISEN7:1, GISEN6:1, GISEN5:1, GISEN4:1, GISEN3:1, GISEN2:1, GISEN1:1,
            % GPIO_ISEN
            GISEN0:1>>
    );
reg(encode, gpio_imod, Val) ->
    #{
        ...
        Val,
    reverse(
        <<2#0:23, GIMOD8:1, GIMOD7:1, GIMOD6:1, GIMOD5:1, GIMOD4:1, GIMOD3:1, GIMOD2:1, GIMOD1:1,
            % GPIO_IMOD
            GIMOD0:1>>
    );
reg(encode, gpio_ibes, Val) ->
    #{
        ...
        Val,
    reverse(
        <<2#0:23, GIBES8:1, GIBES7:1, GIBES6:1, GIBES5:1, GIBES4:1, GIBES3:1, GIBES2:1, GIBES1:1,
            % GPIO_IBES
            GIBES0:1>>
    );
reg(encode, gpio_iclr, Val) ->
    #{
        ...
        Val,
    reverse(
        <<2#0:23, GICLR8:1, GICLR7:1, GICLR6:1, GICLR5:1, GICLR4:1, GICLR3:1, GICLR2:1, GICLR1:1,
            % GPIO_ICLR
            GICLR0:1>>
    );
reg(encode, gpio_idbe, Val) ->
    #{
        ...
        Val,
    reverse(
        <<2#0:23, GIDBE8:1, GIDBE7:1, GIDBE6:1, GIDBE5:1, GIDBE4:1, GIDBE3:1, GIDBE2:1, GIDBE1:1,
            % GPIO_IDBE
            GIDBE0:1>>
    );
reg(encode, gpio_raw, Val) ->
    #{
        ...
        Val,
    reverse(
        <<2#0:23, GRAWP8:1, GRAWP7:1, GRAWP6:1, GRAWP5:1, GRAWP4:1, GRAWP3:1, GRAWP2:1, GRAWP1:1,
            % GPIO_RAW
            GRAWP0:1>>
    );
reg(decode, gpio_ctrl, Resp) ->
    <<_:23, GRAWP8:1, GRAWP7:1, GRAWP6:1, GRAWP5:1, GRAWP4:1, GRAWP3:1, GRAWP2:1, GRAWP1:1,
        % GPIO_RAW
        GRAWP0:1, _:23, GIDBE8:1, GIDBE7:1, GIDBE6:1, GIDBE5:1, GIDBE4:1, GIDBE3:1, GIDBE2:1, GIDBE1:1,
        % GPIO_IDBE
        GIDBE0:1, _:23, GICLR8:1, GICLR7:1, GICLR6:1, GICLR5:1, GICLR4:1, GICLR3:1, GICLR2:1, GICLR1:1,
        % GPIO_ICLR
        GICLR0:1, _:23, GIBES8:1, GIBES7:1, GIBES6:1, GIBES5:1, GIBES4:1, GIBES3:1, GIBES2:1, GIBES1:1,
        % GPIO_IBES
        GIBES0:1, _:23, GIMOD8:1, GIMOD7:1, GIMOD6:1, GIMOD5:1, GIMOD4:1, GIMOD3:1, GIMOD2:1, GIMOD1:1,
        % GPIO_IMOD
        GIMOD0:1, _:23, GISEN8:1, GISEN7:1, GISEN6:1, GISEN5:1, GISEN4:1, GISEN3:1, GISEN2:1, GISEN1:1,
        % GPIO_ISEN
        GISEN0:1, _:23, GIRQE8:1, GIRQE7:1, GIRQE6:1, GIRQE5:1, GIRQE4:1, GIRQE3:1, GIRQE2:1, GIRQE1:1,
        % GPIO_IRQE
        GIRQE0:1, _:11, GOM8:1, _:3, GOP8:1, GOM7:1, GOM6:1, GOM5:1, GOM4:1, GOP7:1, GOP6:1, GOP5:1, GOP4:1, GOM3:1, GOM2:1, GOM1:1, GOM0:1, GOP3:1, GOP2:1, GOP1:1,
        % GPIO_DOUT
        GOP0:1, _:11, GDM8:1, _:3, GDP8:1, GDM7:1, GDM6:1, GDM5:1, GDM4:1, GDP7:1, GDP6:1, GDP5:1, GDP4:1, GDM3:1, GDM2:1, GDM1:1, GDM0:1, GDP3:1, GDP2:1, GDP1:1,
        % GPIO_DIR
        GDP0:1,
        % Reserved
        _:32, _:8, MSGP8:2, MSGP7:2, MSGP6:2, MSGP5:2, MSGP4:2, MSGP3:2, MSGP2:2, MSGP1:2, MSGP0:2,
        % GPIO_MODE
        _:6>> =
        reverse(Resp),
    #{
...
reg(encode, drx_tune0b, Val) ->
    #{drx_tune0b := DRX_TUNE0b} = Val,
    reverse(<<DRX_TUNE0b:16>>);
reg(encode, drx_tune1a, Val) ->
    #{drx_tune1a := DRX_TUNE1a} = Val,
    reverse(<<DRX_TUNE1a:16>>);
reg(encode, drx_tune1b, Val) ->
    #{drx_tune1b := DRX_TUNE1b} = Val,
    reverse(<<DRX_TUNE1b:16>>);
reg(encode, drx_tune2, Val) ->
    #{drx_tune2 := DRX_TUNE2} = Val,
    reverse(<<DRX_TUNE2:32>>);
reg(encode, drx_sfdtoc, Val) ->
    #{drx_sfdtoc := DRX_SFDTOC} = Val,
    reverse(<<DRX_SFDTOC:16>>);
reg(encode, drx_pretoc, Val) ->
    #{drx_pretoc := DRX_PRETOC} = Val,
    reverse(<<DRX_PRETOC:16>>);
reg(encode, drx_tune4h, Val) ->
    #{drx_tune4h := DRX_TUNE4H} = Val,
    reverse(<<DRX_TUNE4H:16>>);
reg(decode, drx_conf, Resp) ->
    % present in the user manual but not in the driver code in C
    <<RXPACC_NOSAT:8,
        % _:8, % Placeholder for the remaining 8 bits
        DRX_CAR_INT:24, DRX_TUNE4H:16, DRX_PRETOC:16, _:16, DRX_SFDTOC:16, _:160, DRX_TUNE2:32, DRX_TUNE1b:16, DRX_TUNE1a:16, DRX_TUNE0b:16,
        _:16>> =
        reverse(Resp),
    #{
...
reg(encode, rf_conf, Val) ->
    #{
        ...
        Val,
    reverse(
        <<2#0:9, TXRXSW:2, LDOFEN:5, PLLFEN:3, TXFEN:5,
            % RF_CONF
            2#0:8>>
    );
reg(encode, rf_rxctrlh, Val) ->
    #{rf_rxctrlh := RF_RXCTRLH} = Val,
    % RF_RXCTRLH
    reverse(<<RF_RXCTRLH:8>>);
% user manual gives fields but encoding should be done as one following table 38
reg(encode, rf_txctrl, Val) ->
    #{rf_txctrl := RF_TXCTRL} = Val,
    reverse(<<RF_TXCTRL:32>>);
reg(encode, ldotune, Val) ->
    #{ldotune := LDOTUNE} = Val,
    reverse(<<LDOTUNE:40>>);
reg(decode, rf_conf, Resp) ->
    % Placeholder for the remaining 40 bits
    <<_:40,
        % LDOTUNE
        LDOTUNE:40, _:28, RFPLLLOCK:1, CPLLHIGH:1, CPLLLOW:1,
        % RF_STATUS
        CPLLLOCK:1, _:128,
        % Reserved 2 - On user manual 16 bytes but offset gives 28 bytes (16 bytes (128 bits) + 12 bytes (96 bits))
        _:96,
        % cf. encode function: Reserved:20, TXMQ:3, TXMTUNE:4, _:5 - RF_TXCTRL
        RF_TXCTRL:32,
        % RF_RXCTRLH
        RF_RXCTRLH:8,
        % Reserved 1
        _:56, _:9, TXRXSW:2, LDOFEN:5, PLLFEN:3, TXFEN:5,
        % RF_CONF
        _:8>> =
        reverse(Resp),
    #{
...
reg(encode, tc_sarc, Val) ->
    #{sar_ctrl := SAR_CTRL} = Val,
    reverse(<<2#0:15, SAR_CTRL:1>>);
reg(encode, tc_pg_ctrl, Val) ->
    #{
        ...
        Val,
    reverse(<<2#0:2, PG_TMEAS:4, Reserved:1, PG_START:1>>);
reg(encode, tc_pgdelay, Val) ->
    #{tc_pgdelay := TC_PGDELAY} = Val,
    reverse(<<TC_PGDELAY:8>>);
reg(encode, tc_pgtest, Val) ->
    #{tc_pgtest := TC_PGTEST} = Val,
    reverse(<<TC_PGTEST:8>>);
reg(decode, tx_cal, Resp) ->
    ...
    <<TC_PGTEST:8,
        ...
        TC_PGDELAY:8, _:4,
        % TC_PG_STATUS
        DELAY_CNT:12, _:2, PG_TMEAS:4, Reserved0:1,
        % TC_PG_CTRL
        PG_START:1, SAR_WTEMP:8,
        % TC_SARW
        SAR_WVBAT:8, _:8, SAR_LTEMP:8,
        % TC_SARL
        SAR_LVBAT:8,
        % Place holder to fill the gap between the offsets
        _:8, _:15,
        % TC_SARC
        SAR_CTRL:1>> =
        reverse(Resp),
...
reg(encode, fs_pllcfg, Val) ->
    #{fs_pllcfg := FS_PLLCFG} = Val,
    reverse(<<FS_PLLCFG:32>>);
reg(encode, fs_plltune, Val) ->
    #{fs_plltune := FS_PLLTUNE} = Val,
    reverse(<<FS_PLLTUNE:8>>);
reg(encode, fs_xtalt, Val) ->
    #{res := Reserved, xtalt := XTALT} = Val,
    reverse(<<Reserved:3, XTALT:5>>);
...
reg(encode, aon_wcfg, Val) ->
    #{
        ...
        Val,
    reverse(
        <<2#0:3, ONW_LLD:1, ONW_LLDE:1, 2#0:2, PRES_SLEE:1, OWN_L64:1, OWN_LDC:1, 2#0:2, OWN_LEUI:1, 2#0:1, OWN_RX:1,
            % AON_WCFG
            OWN_RAD:1>>
    );
reg(encode, aon_ctrl, Val) ->
    #{
        ...
        Val,
    reverse(
        <<DCA_ENAB:1, 2#0:3, DCA_READ:1, UPL_CFG:1, SAVE:1,
            % AON_CTRL
            RESTORE:1>>
    );
reg(encode, aon_rdat, Val) ->
    #{aon_rdat := AON_RDAT} = Val,
    % AON_RDAT
    reverse(<<AON_RDAT:8>>);
reg(encode, aon_addr, Val) ->
    #{aon_addr := AON_ADDR} = Val,
    % AON_ADDR
    reverse(<<AON_ADDR:8>>);
reg(encode, aon_cfg0, Val) ->
    #{
        ...
        Val,
    reverse(
        <<SLEEP_TIM:16, LPCLKDIVA:11, LPDIV_EN:1, WAKE_CNT:1, WAKE_SPI:1, WAKE_PIN:1,
            % AON_CFG0
            SLEEP_EN:1>>
    );
...
reg(decode, aon, Resp) ->
    <<Reserved:13, LPOSC_C:1, SMXX:1,
        % AON_CFG1
        SLEEP_CE:1, SLEEP_TIM:16, LPCLKDIVA:11, LPDIV_EN:1, WAKE_CNT:1, WAKE_SPI:1, WAKE_PIN:1,
        % AON_CFG0
        SLEEP_EN:1,
        % Reserved 1
        _:8,
        ...
        AON_ADDR:8,
        % AON_RDAT
        AON_RDAT:8, DCA_ENAB:1, _:3, DCA_READ:1, UPL_CFG:1, SAVE:1,
        % AON_CTRL
        RESTORE:1, _:3, ONW_LLD:1, ONW_LLDE:1, _:2, PRES_SLEE:1, OWN_L64:1, OWN_LDC:1, _:2, OWN_LEUI:1, _:1, OWN_RX:1,
        % AON_WCFG
        OWN_RAD:1>> =
        reverse(Resp),
    #{
...
reg(encode, otp_wdat, Val) ->
    #{otp_wdat := OTP_WDAT} = Val,
    % OTP_WDAT
    reverse(<<OTP_WDAT:32>>);
reg(encode, otp_addr, Val) ->
    #{otpaddr := OTP_ADDR, res := Reserved} = Val,
    reverse(
        <<Reserved:5,
            % OTP_ADDR
            OTP_ADDR:11>>
...
reg(encode, otp_ctrl, Val) ->
    #{
        ...
        Val,
    reverse(
        <<LDELOAD:1, Reserved1:4, OTPMR:4, OTPPROG:1, Reserved2:2, OTPMRWR:1, Reserved3:1, OTPREAD:1,
            % OTP_CTRL
            OTPRDEN:1>>
    );
reg(encode, otp_stat, Val) ->
    #{
        ...
        Val,
    reverse(
        <<Reserved:14, OTP_VPOK:1,
            % OTP_STAT
            OTPPRGD:1>>
    );
reg(encode, otp_rdat, Val) ->
    #{otp_rdat := OTP_RDAT} = Val,
    % OTP_RDAT
    reverse(<<OTP_RDAT:32>>);
reg(encode, opt_srdat, Val) ->
    #{otp_srdat := OTP_SRDAT} = Val,
    % OTP_SRDAT
    reverse(<<OTP_SRDAT:32>>);
reg(encode, otp_sf, Val) ->
    #{
        ...
        Val,
    reverse(
        <<Reserved1:2, OPS_SEL:1, Reserved2:3, LDO_KICK:1,
            % OTP_SF
            OPS_KICK:1>>
    );
reg(decode, otp_if, Resp) ->
    <<Reserved5:2, OPS_SEL:1, Reserved6:3, LDO_KICK:1,
        % OTP_SF
        OPS_KICK:1,
        % OTP_SRDAT
        OTP_SRDAT:32,
        % OTP_RDAT
        OTP_RDAT:32, Reserved4:14, OTP_VPOK:1,
        % OTP_STAT
        OTPPRGD:1, LDELOAD:1, Reserved1:4, OTPMR:4, OTPPROG:1, Reserved2:2, OTPMRWR:1, Reserved3:1, OTPREAD:1,
        % OTP_CTRL
        OTPRDEN:1, Reserved0:5,
        % OTP_ADDR
        OTP_ADDR:11,
        % OTP_WDAT
        OTP_WDAT:32>> =
...
reg(encode, pmsc_ctrl0, Val) ->
    #{
        ...
        rxclks := RXCLKS,
        sysclks :=
            % Here we need res8 for the initial config of the DW1000. We need to write it
            SYSCLKS
    } =
        Val,
    reverse(
        <<SOFTRESET:4, 2#000:3, PLL2_SEQ_EN:1, KHZCLKEN:1, 2#011:3, GPDRN:1, GPDCE:1, GPRN:1, GPCE:1, AMCE:1, 2#0000:4, ADCCE:1, OTP:1, Res8:1, Res7:1, FACE:1, TXCLKS:2, RXCLKS:2,
            % PMSC_CTRL0
            SYSCLKS:2>>
    );
reg(encode, pmsc_ctrl1, Val) ->
    #{
        ...
        Val,
    reverse(
        <<KHZCLKDIV:6, 2#01000000:8, LDERUNE:1, 2#0:1, PLLSYN:1, SNOZR:1, SNOZE:1, ARXSLP:1, ATXSLP:1, PKTSEQ:8, 2#0:1, ARX2INIT:1,
            % PMSC_CTRL1
            2#0:1>>
    );
reg(encode, pmsc_snozt, Val) ->
    #{snoz_tim := SNOZ_TIM} = Val,
    % PMSC_SNOZT
    reverse(<<SNOZ_TIM:8>>);
reg(encode, pmsc_txfseq, Val) ->
    #{txfineseq := TXFINESEQ} = Val,
    % PMSC_TXFINESEQ
    reverse(<<TXFINESEQ:16>>);
reg(encode, pmsc_ledc, Val) ->
    #{
        ...
        Val,
    reverse(
        <<RES31:12, BLNKNOW:4, RES15:7, BLNKEN:1,
            % PMSC_LEDC
            BLINK_TIM:8>>
    );
...
reg(decode, pmsc, Resp) ->
    ...
    <<Res31:12, BLNKNOW:4, Res15:7, BLNKEN:1,
        ...
        SNOZ_TIM:8,
        % Reserved 1
        _:32, KHZCLKDIV:6, _:8, LDERUNE:1, _:1, PLLSYN:1, SNOZR:1, SNOZE:1, ARXSLP:1, ATXSLP:1, PKTSEQ:8, _:1, ARX2INIT:1,
        % PMSC_CTRL1
        _:1, SOFTRESET:4, _:3, PLL2_SEQ_EN:1, KHZCLKEN:1, _:3, GPDRN:1, GPDCE:1, GPRN:1, GPCE:1, AMCE:1, _:4, ADCCE:1, OTP:1, Res8:1, Res7:1, FACE:1, TXCLKS:2, RXCLKS:2,
        % PMSC_CTRL0
        SYSCLKS:2>> =
        reverse(Resp),
    #{
...
reg(decode, RegFile, Resp) ->
    error({unknown_regfile_to_decode, RegFile, Resp});
reg(encode, RegFile, Resp) ->
    error({unknown_regfile_to_encode, RegFile, Resp}).
...
regSize(dx_time) ->
    5;
regSize(rx_fwto) ->
    % user manual gives 2 bytes and bits 16-31 are reserved
    2;
regSize(sys_ctrl) ->
    4;
regSize(sys_mask) ->
...
regSize(gpio_ctrl) ->
    44;
regSize(drx_conf) ->
    % user manual gives 44 bytes but sum of register length gives 45 bytes
    44;
regSize(rf_conf) ->
    % user manual gives 58 but sum of all its register gives 53 => Placeholder for the remaining 8 bytes
    58;
regSize(tx_cal) ->
    % user manual gives 52 bytes but sum of all sub regs gives 13 bytes
    13;
regSize(fs_ctrl) ->
    21;
regSize(aon) ->
    12;
regSize(otp_if) ->
    % user manual gives 18 bytes in regs table but sum of all sub regs is 19 bytes
    19;
regSize(lde_ctrl) ->
    % No size ?
    undefined;
regSize(lde_if) ->
    ...
    undefined;
regSize(dig_diag) ->
    % user manual gives 41 bytes but sum of all sub regs gives 38 bytes
    38;
regSize(pmsc) ->
    % user manual gives 48 bytes but sum of all sub regs gives 41 bytes
    44.

%% Gives the size in bytes
subRegSize(agc_ctrl1) ->
    2;
subRegSize(agc_tune1) ->
    2;
subRegSize(agc_tune2) ->
    4;
subRegSize(agc_tune3) ->
    2;
subRegSize(agc_stat1) ->
    3;
subRegSize(ec_ctrl) ->
    4;
subRegSize(gpio_mode) ->
    4;
subRegSize(gpio_dir) ->
    4;
subRegSize(gpio_dout) ->
    4;
subRegSize(gpio_irqe) ->
    4;
subRegSize(gpio_isen) ->
    4;
subRegSize(gpio_imode) ->
    4;
subRegSize(gpio_ibes) ->
    4;
subRegSize(gpio_iclr) ->
    4;
subRegSize(gpio_idbe) ->
    4;
subRegSize(gpio_raw) ->
    4;
subRegSize(drx_tune0b) ->
    2;
subRegSize(drx_tune1a) ->
    2;
subRegSize(drx_tune1b) ->
    2;
subRegSize(drx_tune2) ->
    4;
subRegSize(drx_sfdtoc) ->
    2;
subRegSize(drx_pretoc) ->
    2;
subRegSize(drx_tune4h) ->
    2;
subRegSize(rf_conf) ->
    4;
subRegSize(rf_rxctrlh) ->
    1;
subRegSize(rf_txctrl) ->
    % ! table in user manual gives 3 but details gives 4
    4;
subRegSize(ldotune) ->
    5;
subRegSize(tc_sarc) ->
    2;
subRegSize(tc_pg_ctrl) ->
    1;
subRegSize(tc_pgdelay) ->
    1;
subRegSize(tc_pgtest) ->
    1;
subRegSize(fs_pllcfg) ->
    4;
subRegSize(fs_plltune) ->
    1;
subRegSize(fs_xtalt) ->
    1;
subRegSize(aon_wcfg) ->
...
subRegSize(otp_rdat) ->
    4;
subRegSize(otp_srdat) ->
    4;
subRegSize(otp_sf) ->
    1;
subRegSize(lde_thresh) ->
    2;
subRegSize(lde_cfg1) ->
    1;
subRegSize(lde_ppindx) ->
    2;
subRegSize(lde_ppampl) ->
    2;
subRegSize(lde_rxantd) ->
    2;
subRegSize(lde_cfg2) ->
    2;
subRegSize(lde_repc) ->
...
subRegSize(diag_tmc) ->
    2;
subRegSize(pmsc_ctrl0) ->
    4;
subRegSize(pmsc_ctrl1) ->
    4;
subRegSize(pmsc_snozt) ->
    1;
subRegSize(pmsc_txfseq) ->
    2;
subRegSize(pmsc_ledc) ->
    4;
subRegSize(_) ->
    error({error}).
...
debug_read(Reg, Value) ->
    io:format(
        "[PmodUWB] read [16#~2.16.0B - ~w] --> ~s -> ~s~n",
        [regFile(Reg), Reg, debug_bitstring(Value), debug_bitstring_hex(Value)]
    ).

debug_write(Reg, Value) ->
    io:format(
        "[PmodUWB] write [16#~2.16.0B - ~w] --> ~s -> ~s~n",
        [regFile(Reg), Reg, debug_bitstring(Value), debug_bitstring_hex(Value)]
    ).

debug_write(Reg, SubReg, Value) ->
    io:format(
        "[PmodUWB] write [16#~2.16.0B - ~w - 16#~2.16.0B - ~w] --> ~s -> ~s~n",
        [
            regFile(Reg),
            Reg,
            subReg(SubReg),
            SubReg,
            debug_bitstring(Value),
